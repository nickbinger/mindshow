#!/usr/bin/env python3
"""
Unified Brainwave System with Real Muse Data
Feeds real brainwave data to both web dashboard and GUI simultaneously
"""
...
# Global data queue for sharing real brainwave data
brainwave_queue = queue.Queue()
...
class SimpleGUI:
    """Simple Tkinter GUI for brain state display"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("MindShow Brain State Monitor")
    ...
    def setup_gui(self):
        """Setup GUI elements"""
        # Title
        title_label = tk.Label(self.root, text="🧠 MindShow Brain State", 
                             font=("Arial", 16, "bold"), 
                             fg="white", bg="black")
        ...
        # Scores Frame
        scores_frame = tk.Frame(self.root, bg="black")
        scores_frame.pack(pady=10)
        
        # Attention Score
        attention_frame = tk.Frame(scores_frame, bg="black")
        attention_frame.pack(pady=5)
        tk.Label(attention_frame, text="Attention:", font=("Arial", 12), 
                fg="white", bg="black").pack(side=tk.LEFT)
        self.attention_label = tk.Label(attention_frame, text="0.000", 
                                      font=("Arial", 12, "bold"), 
                                      fg="red", bg="black")
        self.attention_label.pack(side=tk.LEFT, padx=10)
        
        # Relaxation Score
        relaxation_frame = tk.Frame(scores_frame, bg="black")
        relaxation_frame.pack(pady=5)
        tk.Label(relaxation_frame, text="Relaxation:", font=("Arial", 12), 
                fg="white", bg="black").pack(side=tk.LEFT)
        self.relaxation_label = tk.Label(relaxation_frame, text="0.000", 
                                       font=("Arial", 12, "bold"), 
                                       fg="blue", bg="black")
        self.relaxation_label.pack(side=tk.LEFT, padx=10)
    ...
    def update_display(self, brain_data):
        ...
        self.state_label.config(text=f"State: {brain_state.upper()}")
        ...
        # Update scores
        self.attention_label.config(text=f"{brain_data['attention']:.3f}")
        self.relaxation_label.config(text=f"{brain_data['relaxation']:.3f}")
...
app = FastAPI(title="MindShow Unified Dashboard")
...
async def root():
    ...
    <html>
    <head>
        <title>MindShow Unified Dashboard</title>
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>🧠 MindShow Unified Dashboard</h1>
                <p>Real-time brainwave visualization with LED control</p>
            </div>
        ...
        <script>
            ...
            ws.onopen = function() {
                document.getElementById('connection-status').textContent = 'Connected - Receiving real brainwave data';
...
async def main_loop():
    ...
    logger.info("Starting unified brainwave processing...")
...
if __name__ == "__main__":
    ...
    logger.info("Starting MindShow Unified System with Real Muse Data...")
