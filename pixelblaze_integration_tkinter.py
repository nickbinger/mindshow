#!/usr/bin/env python3
"""
MindShow EEG LED Hat - Pixelblaze Integration with Tkinter GUI
...
class BrainwaveGUI:
    """Real-time GUI for brainwave visualization using Tkinter"""
    ...
    def setup_gui(self):
        ...
        # Brain State Frame
        state_frame = tk.Frame(self.root, bg='#2c3e50')
        ...
        instructions = [
            "🔵 Relax your mind → Blue LEDs",
            "🔴 Focus/engage → Red LEDs", 
            "🟢 Neutral state → Green LEDs",
    ...
    def update_display(self, brain_state, attention_score, relaxation_score):
        ...
        # Update status
        self.status_display.config(text=f"Status: Running - Last update: {time.strftime('%H:%M:%S')}")
    
    def update_from_queue(self):
        """Update display from data queue"""
        try:
            while not self.data_queue.empty():
                data = self.data_queue.get_nowait()
                attention_score, relaxation_score, brain_state = data
                self.update_display(brain_state, attention_score, relaxation_score)
        except queue.Empty:
            pass
    ...
    def add_data(self, attention_score, relaxation_score, brain_state):
        """Add data to the queue for GUI update"""
        self.data_queue.put((attention_score, relaxation_score, brain_state))
...
class MindShowController:
    ...
    async def start(self):
        ...
        logger.info("=== MindShow EEG LED Controller with Native BrainFlow + Tkinter GUI ===")
        ...
        try:
            while self.running:
                ...
                # Update GUI with new data (non-blocking)
                self.gui.add_data(attention_score, relaxation_score, brain_state)
