#!/usr/bin/env python3
"""
MindShow EEG LED Hat - Pixelblaze Integration with Working GUI
Phase 2: Connect brainwave analysis to Pixelblaze LED patterns with working GUI
"""
...
class WorkingBrainwaveGUI:
    """Working real-time GUI for brainwave visualization"""
    ...
    def update_plots(self, frame):
        ...
        try:
            while not self.data_queue.empty():
                ...
                self.add_data_point(attention_score, relaxation_score, brain_state)
        except queue.Empty:
            pass
    ...
    def update_data(self, attention_score, relaxation_score, brain_state):
        """Update data from external thread"""
        self.data_queue.put((attention_score, relaxation_score, brain_state))
    
    def start_gui(self):
        """Start the GUI"""
        self.running = True
        self.ani = FuncAnimation(self.fig, self.update_plots, interval=100, blit=True, cache_frame_data=False)
        plt.show()
...
class MindShowController:
    ...
    def __init__(self):
        ...
        self.gui = WorkingBrainwaveGUI()
        self.running = False
        
    async def start(self):
        """Start the mind-controlled LED system with native BrainFlow analysis and GUI"""
        logger.info("=== MindShow EEG LED Controller with Native BrainFlow + Working GUI ===")
        ...
        try:
            while self.running:
                ...
                self.gui.update_data(attention_score, relaxation_score, brain_state)
